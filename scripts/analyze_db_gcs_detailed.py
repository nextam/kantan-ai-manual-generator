#!/usr/bin/env python3
"""
データベース内ファイルとGCS存在状況の詳細調査（手動リスト版）
"""
import sqlite3
import os
...
def print_analysis_results(results):
    ...
    # 2. 重複ファイル分析（特に欠損があるもの）
    print("🔄 重複ファイル分析（欠損があるもの）:")
    ...
    for original_name, versions in duplicates.items():
        ...
        if missing_versions:  # 欠損があるもののみ表示
            print(f"\n📄 元ファイル名: {original_name}")
            print(f"   バージョン数: {len(versions)} (存在: {len(existing_versions)}, 欠損: {len(missing_versions)})")
            
            if existing_versions:
                print("   ✅ GCS存在バージョン:")
                for v in existing_versions:
                    print(f"      ID {v['id']}: {v['path']}")
            ...
            if missing_versions:
                ...
                for v in missing_versions:
                    ...
                    # 代替候補を提案
                    if existing_versions:
                        recommended = existing_versions[0]  # 最初の存在するファイル
                        print(f"      💡 代替候補: ID {recommended['id']} -> {recommended['path']}")
